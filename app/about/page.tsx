...
const ICON_MAP: Record<string, React.ComponentType<{ size?: number; 'aria-hidden'?: boolean; className?: string }>> = {
  'user-check': UserCheck, 'users': Users, 'globe': Globe,
};

const FALLBACK_CAREER_THREAD = [
  { n: '01', title: 'Cell & Gene Therapy Manufacturing',    body: 'Hands-on operations in advanced-therapy production — patient-specific batches, chain of identity, and scale-up under evolving ATMP frameworks.' },
  { n: '02', title: 'Pharmaceutical Quality Systems',       body: 'Building and modernising GxP quality systems — the discipline that turns a process into something a regulator will approve.' },
  { n: '03', title: 'AI Strategy in Regulated Environments', body: 'Defining where AI creates durable value when governance, validation and data integrity are non-negotiable.' },
  { n: '04', title: 'Digital Operations Transformation',    body: 'Moving manufacturing and quality beyond paper and legacy systems toward an AI-ready operational data layer.' },
  { n: '05', title: 'Vision for AI-Enabled Biotech Operations', body: 'A through-line conviction: the next decade of biotech operations is defined by firms that operationalise AI without compromising rigour.' },
];

const FALLBACK_HOW_WE_WORK = [
  { iconKey: 'user-check', label: 'Delivery',  value: 'Senior-led, principal-delivered' },
  { iconKey: 'users',      label: 'Roster',    value: 'Small, deliberately' },
  { iconKey: 'globe',      label: 'Geography', value: 'UK · United States · Europe' },
];

export default async function AboutPage() {
  const cms = await sanityFetch<AboutPage>(aboutPageQuery);

  const careerThread = cms?.careerThread ?? FALLBACK_CAREER_THREAD;
  const HOW_WE_WORK  = cms?.howWeWork    ?? FALLBACK_HOW_WE_WORK;
  const hero         = cms?.hero;
    ...
    <main>
      ...
      >
        ...
        <div className="relative max-w-[1240px] mx-auto px-6 md:px-10 pt-20 pb-16">
          ...
          <h1 className="font-sans font-semibold text-[clamp(2.25rem,3.5vw,4rem)] leading-[1.08] tracking-[-0.02em] mt-5 mb-5 max-w-[18ch]">
            {hero?.heading ?? 'About DigiSci and its Founder'}
          </h1>
          <p className="font-serif text-[1.125rem] leading-[1.65] text-[var(--text-secondary)] max-w-[46em]">
            {hero?.body ?? "DigiSci was built on a straightforward observation: the biotech industry needs transformation partners who understand both the science of the operation and the science of AI."}
          </p>
        </div>
      </section>
      ...
      <section aria-label="Founder profile" className="border-b border-[var(--border-subtle)]">
        <div className="max-w-[1240px] mx-auto px-6 md:px-10 py-24">
          <div className="grid grid-cols-1 gap-10 md:grid-cols-[320px_1fr] md:gap-16 items-start">
            ...
            >
              <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,var(--blue-glow),transparent_65%)]" />
              <span className="relative font-mono text-[0.6875rem] tracking-[0.14em] uppercase text-[var(--text-secondary)]">
                {founder?.name ?? 'Kwok Pang'} · Founder &amp; Principal
              </span>
            </div>
            ...
            <div>
              ...
              <h2 className="font-sans font-semibold text-[2.5rem] leading-[1.1] tracking-[-0.02em] mt-4 mb-6 max-w-[20ch]">
                {founder?.name ?? 'Kwok Pang'} — {founder?.title ?? 'Operator, Digital Strategist, AI Transformation Architect.'}
              </h2>
              ...
              <blockquote className="font-sans text-[1.75rem] leading-[1.45] text-[var(--text-primary)] font-medium border-l-2 border-[var(--accent)] pl-5 mt-8">
                &quot;{founder?.quote ?? 'The most valuable transformation work happens when deep domain expertise and AI capability are held in the same mind.'}&quot;
              </blockquote>
              <div className="mt-6 flex gap-3">
                <Button variant="secondary" as="a" href="https://www.linkedin.com/in/kwok-pang" iconRight={<LinkedinIcon size={16} />}>
                  Connect on LinkedIn
                </Button>
              </div>
            </div>
      ...
      <section aria-label="Career thread" className="bg-[var(--surface-sunken)] border-t border-b border-[var(--border-subtle)]">
        <div className="max-w-[1240px] mx-auto px-6 md:px-10 py-24">
          <div className="max-w-[44rem] mb-10">
            ...
            <h2 className="font-sans font-semibold text-[3rem] leading-[1.1] tracking-[-0.02em] mt-4">The through-line is the proof.</h2>
            <p className="font-serif text-[1.125rem] leading-[1.65] text-[var(--text-secondary)] mt-4">Each area of expertise maps directly to a capability DigiSci sells.</p>
          </div>
          <ul className="list-none p-0 m-0">
            {careerThread.map(({ n, title, body }) => (
              <li key={n} className="grid grid-cols-[56px_1fr] gap-5 py-6 border-t border-[var(--border-subtle)] items-baseline">
                <span className="font-mono text-sm text-[var(--text-accent)]">{n}</span>
                <div>
                  <h3 className="font-sans font-semibold text-[1.375rem] text-[var(--text-primary)] mb-2">{title}</h3>
      ...
      >
        <div className="max-w-[1240px] mx-auto px-6 md:px-10">
          <Eyebrow rule>{mission?.eyebrow ?? 'The mission'}</Eyebrow>
          <p className="font-sans font-semibold text-[4rem] leading-[1.08] tracking-[-0.02em] max-w-[18ch] mx-auto mt-5">
            {mission?.heading ?? 'Enable the'}{' '}
            <b className="text-[var(--accent)] font-semibold">{mission?.accentText ?? 'AI-driven future'}</b>
            {mission?.heading ? '' : ' of biotech operations.'}
          </p>
        </div>
      </section>
      ...
      <section aria-label="How we work" className="border-b border-[var(--border-subtle)]">
        <div className="max-w-[1240px] mx-auto px-6 md:px-10 py-24 grid grid-cols-1 gap-16 md:grid-cols-[0.8fr_1.2fr] md:items-start">
          <div>
            ...
            <h2 className="font-sans font-semibold text-[3rem] leading-[1.1] tracking-[-0.02em] mt-4 max-w-[16ch]">A focused, senior-led boutique.</h2>
          </div>
          <div>
            ...
            <div className="flex flex-col gap-5 mt-6 pt-6 border-t border-[var(--border-subtle)] sm:flex-row sm:flex-wrap sm:gap-6">
              {HOW_WE_WORK.map(({ iconKey, label, value }) => {
                const Icon = ICON_MAP[iconKey] ?? UserCheck;
                return (
                  <div key={label} className="flex gap-3 items-start">
                    <Icon size={18} aria-hidden className="text-[var(--accent)] shrink-0 mt-[2px]" />
                    <div>
                      <span className="font-mono text-[0.6875rem] tracking-[0.14em] uppercase text-[var(--text-tertiary)] block mb-1">{label}</span>
                      <span className="font-sans text-sm text-[var(--text-primary)]">{value}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <section aria-label="Call to action" className="bg-[var(--surface-sunken)] border-t border-[var(--border-subtle)]">
        <div className="max-w-[1240px] mx-auto px-6 md:px-10 py-24 flex flex-col gap-12 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <Eyebrow rule>{cta?.eyebrow ?? 'Two ways forward'}</Eyebrow>
            <h2 className="font-sans font-semibold text-[3rem] leading-[1.1] tracking-[-0.02em] mt-4 max-w-[22ch]">
              {cta?.heading ?? "Ready to engage, or prefer to research first?"}
            </h2>
            <p className="font-serif text-[1.125rem] leading-[1.65] text-[var(--text-secondary)] mt-4 max-w-[40ch]">
              {cta?.body ?? "About pages have high intent — if you read this far, let's talk. Or follow the work on LinkedIn."}
            </p>
          </div>
          <div className="flex gap-3 shrink-0">
            <Button variant="primary" size="lg" as="a" href="/services" iconRight={<ArrowRight size={17} />}>View Services</Button>
            <Button variant="secondary" size="lg" as="a" href="https://www.linkedin.com/in/kwok-pang">Connect on LinkedIn</Button>
          </div>
        </div>
      </section>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            worksFor: { '@type': 'Organization', name: 'DigiSci', url: 'https://digisci.solutions' },
